...
export function ExecutePopup({ onExecute }: ExecutePopupProps) {
  ...
  const executeCommand = async (commandText: string) => {
    try {
      ...
      // Execute the command using our TypeScript command system
      const result = await execCmd(commandText);

      logger.debug(`Command execution result:`, result);

      // Show success toast with the result
      if (result.success) {
        // Display appropriate success message with action details
        const title = result.message || "Command executed successfully";
        const description = result.action
          ? `Action performed: ${result.action}`
          : undefined;

        toast.success(title, { description });
        logger.info(`Command successful: ${title} - ${description || ""}`);
        ...
        const errorTitle = result.message || "Command execution failed";
        const errorDescription = result.error || "Unknown error occurred";

        toast.error(errorTitle, { description: errorDescription });
        logger.error(`Command failed: ${errorTitle} - ${errorDescription}`);
      }
    } catch (error) {
      ...
      toast.error("Error executing command", { description: errorMsg });
    } finally {
      // Reset states
